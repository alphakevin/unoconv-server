...
class Converter {
  ...
  convert(inputFile, format, outputFile) {
    args = [];
    if (!format) {
      format = 'pdf';
    }
    ...
    if (!outputFile) {
      outputFile = `${path.parse(inputFile).name}.${format}`;
    }
  ...
  convertToStream(args, inputFile) {
    args.push('--stdout');
    args.push(inputFile);
    ...
    const handler = childProcess.spawn('unoconv', args);
    const errors = [];
    handler.stderr.on('data', chunk => errors.push(chunk));
    handler.on('exit', (code) => {
      fs.unlinkSync(inputFile);
      if (code > 0 || errors.length) {
        output.emit('error', Buffer.concat(errors).toString('utf8'));
      }
    });
